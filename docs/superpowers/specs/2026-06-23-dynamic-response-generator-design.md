...
**类型规则**（关键）：
- **纯表达式** — string value 整个就是单个 `{{...}}`（无前后文本、无引号外字符）：
  - 生成器输出按其声明的 `outputType` **原类型注入**
  - 例：`"{{$int:1:100}}"` → `42`（number）
  - 例：`"{{$uuid}}"` → `"a3f1-..."`（string，因为 uuid 的 `outputType` 是 string）
  - 例：`"{{$bool}}"` → `true`（boolean）
- **混合表达式** — string value 含文本 + 表达式（或多个表达式）：
  - 一律 `String(...)` 拼接成 string
  - 例：`"id-{{$int:1:100}}"` → `"id-42"`
  - 例：`"{{$uuid}}-{{$uuid}}"` → `"a3f1-...-b2c3-..."`
- **失败**：未知 ID / 参数错 → 纯表达式 → `null`；混合 → 保留原字符串（让用户看到坏表达式）

判断「纯 vs 混合」的算法：`value.length` 去除两端空白后严格等于一个 `{{...}}` 表达式（不允许任何其他字符、包括空格）。
...
export const GENERATORS = {
  uuid: {
    category: 'string',
    label: 'UUID v4',
    outputType: 'string',          // 注入到 JSON 时的目标类型
    args: [],
...
每个生成器必须声明 `outputType`：`'string' | 'number' | 'boolean' | 'date'`。Resolver 根据这个 + 是否纯表达式判断如何注入。
...
| Category | Generators（举例） | outputType |
|---|---|---|
| string | `uuid`, `string.alphanumeric`, `string.nanoid`, `string.symbol` | string |
| lorem | `lorem.word`, `lorem.sentence`, `lorem.paragraph` | string |
| number | `int`, `float` | number |
| date | `date`, `date.recent`, `date.past`, `date.future` | date (输出 ISO string，纯表达式时也是 string) |
| person | `person.fullName`, `person.firstName`, `person.lastName`, `person.gender`, `person.jobTitle` | string |
| phone | `phone.number` | string |
| internet | `internet.email`, `internet.url`, `internet.domainName`, `internet.ip`, `internet.userName`, `internet.password` | string |
| image | `image.url`, `image.avatar`, `image.dataUri` | string |
| location | `location.street`, `location.city`, `location.country`, `location.zipCode` | string |

注意：v1 的 ~30 个生成器里只有 `int` 和 `float` 是 `outputType: number`，其余都是 `string`。这是 faker 本身的数据形态决定 —— 后续如需 `boolean` 生成器可加一条 `bool` 用 `faker.datatype.boolean()`。
...
类型遵循 §3 规则 —— 纯表达式保留生成器 `outputType`（`int` → number），混合表达式为 string。
...
| 限制 | 原因 | 后续路径 |
|---|---|---|
| 混合表达式 → string；纯表达式保留 `outputType` | 表达式语法存储在 string 内，只能靠「是否纯表达式」推断 | 加显式 typed expression：`{{int:1:100}}` 用前缀决定类型 |
...
- `test/unit/expression-resolver.test.js`
  - 标量无表达式：原样
  - **纯表达式（int）→ number**（关键类型保留）
  - **纯表达式（uuid）→ string**
  - **纯表达式（bool）→ boolean**
  - **混合表达式（id-{{$int}}）→ string 拼接**
  - **多表达式（`{{$uuid}}-{{$uuid}}`）→ string 拼接**
  - 嵌套对象/数组：递归
  - object key 含 `{{}}`：不解析
  - 未知 ID：纯 → null + 错误；混合 → 原字符串保留 + 错误
  - 参数类型错：同上分流
  - Faker 抛错：catch → null（纯）/ 原字符串（混合）
  - 空 / null / 非 string
...
| 类型注入规则 | 纯表达式保留 `outputType`；混合 → string | 全部 string | 用户明确要求数字类型不能带引号 |
